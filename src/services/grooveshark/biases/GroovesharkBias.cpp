...
Dynamic::GroovesharkBiasFactory::GroovesharkBiasFactory()
...
QString
Dynamic::GroovesharkBiasFactory::name() const
{

    return i18n( "Grooveshark Similar Artists" );
}

QString
Dynamic::GroovesharkBiasFactory::pluginName() const
{
    return "grooveshark_similarartists";
}



Dynamic::CustomBiasEntry*
Dynamic::GroovesharkBiasFactory::newCustomBiasEntry()
{
    debug() << "CREATING SIMILAR BIAS";
    return new GroovesharkBias( true );
}

Dynamic::CustomBiasEntry*
Dynamic::GroovesharkBiasFactory::newCustomBiasEntry( QDomElement e )
{
    DEBUG_BLOCK
    debug() << "grooveshark bias created with:" << e.attribute("value");
    bool sim = e.attribute( "value" ).toInt() == 0;
    return new GroovesharkBias( sim );
}


/// class GroovesharkBias

Dynamic::GroovesharkBias::GroovesharkBias( bool similarArtists )
    : Dynamic::CustomBiasEntry()
...
Dynamic::GroovesharkBias::~GroovesharkBias()
{
    delete m_qm;
}
...
QString
Dynamic::GroovesharkBias::pluginName() const
{
    return "grooveshark_similarartists";
}

QWidget*
Dynamic::GroovesharkBias::configWidget( QWidget* parent )
{
    DEBUG_BLOCK
    QFrame * frame = new QFrame( parent );
    QVBoxLayout* layout = new QVBoxLayout( frame );

    QLabel * label = new QLabel( i18n( "Adds songs related to currently playing track, recommended by Grooveshark" ), frame );
...
void
Dynamic::GroovesharkBias::activated(int index)
{
...
void Dynamic::GroovesharkBias::updateBias()
{
    DEBUG_BLOCK
    ...
    if( track )
    {
        if( m_similarArtists && track->artist() && !track->artist()->name().isEmpty() )
        {
            ...
            if( !m_savedArtists.contains( track->artist()->name() ) )
            {
                ...
                //m_artistQuery = grooveshark::ws::get( params );
...
// TODO add mbid if the track has one
                ...
                //m_trackQuery = grooveshark::ws::get( params );
...
void
Dynamic::GroovesharkBias::update()
{
    DEBUG_BLOCK
    if( !m_needsUpdating )
        return;
...
void
Dynamic::GroovesharkBias::artistQueryDone() // slot
{
    DEBUG_BLOCK
...
//     debug() << "artistQuery has data:" << data;
    ...
    {
        debug() << "Got invalid XML data from Grooveshark!";
        return;
    }
...
void Dynamic::GroovesharkBias::trackQueryDone()
{
    DEBUG_BLOCK
...
void
Dynamic::GroovesharkBias::updateFinished()
{
    DEBUG_BLOCK
...
void
Dynamic::GroovesharkBias::collectionUpdated()
{
    m_needsUpdating = true;
}

void
Dynamic::GroovesharkBias::artistUpdateReady(QString collectionId, QStringList uids )
{
    addData( collectionId, uids, m_currentArtist, m_savedArtists );
}

void
Dynamic::GroovesharkBias::trackUpdateReady(QString collectionId, QStringList uids )
{
...
void
Dynamic::GroovesharkBias::addData( QString collectionId, QStringList uids, const QString& index, QMap< QString, QSet< QByteArray > >& data )
{
    DEBUG_BLOCK
...
bool
Dynamic::GroovesharkBias::trackSatisfies( const Meta::TrackPtr track )
{
    //DEBUG_BLOCK
    QMutexLocker locker( &m_mutex );
...
double
Dynamic::GroovesharkBias::numTracksThatSatisfy( const Meta::TrackList& tracks )
{
    DEBUG_BLOCK
    QMutexLocker locker( &m_mutex );
...
QDomElement
Dynamic::GroovesharkBias::xml( QDomDocument doc ) const
{
    DEBUG_BLOCK
    QDomElement from = doc.createElement( "similarArtists" );
    from.setAttribute( "value", m_similarArtists ? "0" : "1" );

    debug() << "returning groovesharkbias with xml:" << from.text();

    return from;
}

void
Dynamic::GroovesharkBias::saveDataToFile()
{
    QFile file( Amarok::saveLocation() + "dynamic_grooveshark_similarartists.xml" );
    ...
    QFile file2( Amarok::saveLocation() + "dynamic_grooveshark_similartracks.xml" );
...
void
Dynamic::GroovesharkBias::loadFromFile()
{
    QFile file( Amarok::saveLocation() + "dynamic_grooveshark_similarartists.xml" );
...
bool
Dynamic::GroovesharkBias::hasCollectionFilterCapability()
{
    return true;
}

Dynamic::CollectionFilterCapability*
Dynamic::GroovesharkBias::collectionFilterCapability( double weight )
{
    DEBUG_BLOCK
    debug() << "returning new cfb with weight:" << weight;
    return new Dynamic::GroovesharkBiasCollectionFilterCapability( this, weight );
}

const QSet< QByteArray >&
Dynamic::GroovesharkBiasCollectionFilterCapability::propertySet()
{
...
double Dynamic::GroovesharkBiasCollectionFilterCapability::weight() const
{
    return m_weight;
}

#include "GroovesharkBias.moc"
