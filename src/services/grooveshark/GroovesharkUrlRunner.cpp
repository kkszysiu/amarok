...
GroovesharkUrlRunner::GroovesharkUrlRunner()
 : QObject()
 , AmarokUrlRunnerBase()
{
}
...
QString GroovesharkUrlRunner::command() const
{
    return "service-grooveshark";
}

QString GroovesharkUrlRunner::prettyCommand() const
{
    return i18nc( "A type of command that triggers an action in the integrated Grooveshark.com service", "Grooveshark" );
}

KIcon GroovesharkUrlRunner::icon() const
{
    return KIcon( "view-services-grooveshark-amarok" );
}

bool GroovesharkUrlRunner::run( AmarokUrl url )
{
    DEBUG_BLOCK
    if ( !url.isNull() )
    {
...
#include "GroovesharkUrlRunner.moc"
